...
\date[June 2023]{\\ SEF Sofia Bulgaria \\ June, 2023}
...
\begin{frame}{Summary}
    \begin{itemize}
        ...
        \item \textbf{Design flaws} in widely used markets formats (CDA) and the heterogeneity in access to technology might create an unfair playing field and inneficiencies.
        ...
        \item We study \textbf{convergence} to the underlying competitive equilibrium (CE) price, \textbf{efficiency}, {price dispersion}, \textbf{message intensity} and inequality, etc. 
    \end{itemize}
\end{frame}
...
\begin{frame}{Motivation}
    ...
    \begin{itemize}
        ...
        \item Theory and evidence show that if processing/sending orders is cheap, traders' optimal strategy is to \textbf{shred} orders not to have a \textbf{price impact}.
...
\begin{frame}{Motivation}
    ...
    \begin{itemize}
        ...
        \item The rule is usually based on time priority, creating a comm-tech arms race among traders (massive cooperation dilemma) % BCS: better off without investing in comm-tech
        \item the FLOW format reduces de gains from picking off.
    \end{itemize}
    % \vspace{0.5cm}
    % \begin{itemize}
...
(small share)
    ...
    % \item Budish et al (2020) discusses the existing \textbf{lack of incentives to adopt new market designs} that reduce latency arbitrage and deactivates the high-frequency trading arms race. 
    % budish2020theory
    ...
    %\item Biais, Foucault, and Moinas (2013) \item Dynamic Thin Markets (2015):
        ...
        % \item  Biais, Foucault, and Moinas (2015) show that the ability to privately acquire information leads to an \textbf{over-investment in communication speed}. 
        % biais2015equilibrium
        ...
        \item Aldrich \& Lopez Vargas (2020) compare CDA with Frequent-Batch Auctions (FBA). 
        % aldrich2020experiments
...
\begin{frame}{Experiment Design: Interface - FLOW Animation}
  \animategraphics[loop,controls,width=\linewidth]{10}{workshop/flow_animation/flow_animation-}{0}{419}
\end{frame}
...
\begin{frame}{Experiment Design: Interface - CDA Animation}
  \animategraphics[loop,controls,width=\linewidth]{10}{workshop/cda_animation/cda_animation-}{0}{252}
\end{frame}
...
\begin{frame}{Results: Clearing Prices}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=\textwidth]{figures/groups_cda_price.png}
        \caption{CDA Price}
        \label{fig:price_cda}
    \end{figure}
\end{frame}
...
\begin{frame}{Conclusions (Preliminary)}
    ...
    \item There is shredding in CDA as expected.
    ...
    \item After learning, FLOW markets still show weak price convergence.
    \item After learning, traded volume is higher in FLOW.
    \item Several puzzles... (Volume vs efficiency; UI...)
    \item Elements to be incorporated in experiment: heterogeneous messaging costs, costly reduction of latency, direct format competition...
...
\vspace{1cm}
\small{\centerline{\url{kristian@ucsc.edu}}}
\end{frame}
