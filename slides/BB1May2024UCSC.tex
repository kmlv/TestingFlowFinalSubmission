...
\date[4/27/2024]{\vspace{1cm} \textbf{UCSC Econ Brown Bag Seminar} \\
May 1, 2024}
...
Nor does it test theory per se.  \\ %\pause 
It highlights the \textbf{basic functioning} of the two formats, and how traders deal with \textbf{price impact}. %\pause 
...
Sets stage for later investigations of HFT issues and of package bidding.
...
\begin{frame}{Hypotheses}
    \begin{itemize}
        \item[H1] \textit{Traders will fragment orders more externally
(i.e., submit a larger number of smaller quantity orders) in CDA than in Flow format.} %\pause
\vspace{0.3cm}
        \item[H2] \textit{Price volatility will be lower in Flow format than in CDA format.}%\pause
\vspace{0.3cm}        
        \item[H3] \textit{(a) Trading volume in CDA and Flow formats will be similar. \\ (b) Both will move within each trading block towards the minimum volume that sustains competitive equilibrium.} %\pause
\vspace{0.3cm}        
...
\end{equation}
where $y_{g,t}$ is an outcome  indexed by group (i.e., session) and time, and the $\gamma$ coefficient for dummy variable $\text{FLOW}_{g,t}$ is the treatment effect. %\pause
...
\textbf{First test} of Flow market format, comparing it to CDA in an induced-value setting with finely divisible units. %\pause
\begin{itemize}
