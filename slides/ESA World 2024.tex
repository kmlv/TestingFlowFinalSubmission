...
\date[4/27/2024]{\vspace{1cm} \textbf{BEEC} \\
Jan, 2025}
...
Our paper \textbf{compares CDA and Flow} market formats in a ``simple'' lab environment (induced supply - demand are for a \textbf{large set of units}). 
%\pause

This environment is \textbf{not} designed to study HFTs' incentives. \\ 
%\pause 

It highlights the \textbf{basic functioning} of the formats, and how traders deal with price impact. 
%\pause 

We compare:
\begin{itemize}
\item basic traders' behavior %\pause
...
\item All continuous scaled limit orders ares treated \textbf{symmetrically} and executed \textbf{simultaneously} at the market clearing price %\pause
        ...
        \textbf{fastest trader} or \textbf{the ones that can send massive numbers} of messages to update orders are \textbf{not rewarded}
    \end{itemize}
\end{frame}
