...
\title[Lect4-Flow]{Lecture 4:\\
Flow Markets}
\author[DF]{Daniel Friedman\thanks{Economics Departments, University of California Santa Cruz and University of Essex}}
\useoutertheme{infolines}

\begin{document}

\date[UNSW/Agora]{Market Magic Minicourse\\
June 12, 2024
} 
...
\item 10's of \$billions for sub-millisecond speed advantages --- unproductive rent-seeking, \pause
...
so far \textbf{compares CDA and Flow} market formats in a simple lab environment  with finely divisible asset units. 
\pause

This environment is 
suitable for testing claims 0 and 1, on general implementability and price pressure mitigation. 
\pause

It is \textbf{not} designed to test claims 2 and 3, on HFT/sniping issues and portfolio trading, though it does help set the stage.
\pause 

Basically, we run a horse race (CDA vs Flow formats), not a test of theory.
We \textbf{compare across formats}:
\begin{itemize}
...
\begin{frame}{Hypotheses}
    \begin{itemize}
\item[H1] \textit{Traders will submit a larger number of smaller quantity orders in CDA than in Flow format.} \pause
\vspace{0.3cm}
        \item[H2] \textit{Price volatility will be lower in Flow than in CDA format.}\pause
\vspace{0.3cm}        
        \item[H3] \textit{The CDA and Flow formats will exhibit similar levels of trade volume.}
...
\begin{enumerate}
   ...
   Preliminary \textbf{PPI} also considerably lower.
    \vspace{0.5cm}
    
    \item[R3:] Trade \textbf{Volume is lower in the Flow} Market than in the CDA.  \pause 
    \vspace{0.5cm}
    
    \item[R4:] Flow and CDA markets have \textbf{comparable} informational and allocative \textbf{efficiency}. 

    \item[R5:] (ex post) Buyers excess profits are far larger than Sellers' in CDA, not so much in Flow. 
...
\begin{frame}{Descriptive Stats: Market Performance}
    ...
    % \hline 
     ...
     Std. Dev. of $(\ln P_{t} - \ln P_{t - 1})$ & 0.14      & 0.07      & 0.15      & 0.07      & 0.13      & 0.07     \\    
       PPI ($Q^*=20, q=4$)& 2.96 & 0.59 & 3.09 & 0.71 & 2.83 & 0.48 \\
     \\ [-1.8ex]
     ...
     Alloc. Effic.: $100\Pi$ / $\Pi_{\text{CE}}$         & 84.3      & 81.81     & 79.76     & 77.31     & 88.85     & 86.31     \\
     
     ${100[\Pi^{\text{buy}}}/{\Pi^{\text{buy}}_{\text{CE}}} - {\Pi^{\text{sell}}}/{\Pi^{\text{sell}}_{\text{CE}}]}$ 
     & 53.7      & 11.18     & 27.38     & -5.16     & 80.03     & 27.51     \\
     \\[-1.8ex] 
    \bottomrule 
    \end{tabular}
...
\begin{equation} \label{eq:ppicda}
    PPI_{CDA} = \frac{1}{Q^*}\int_0^{Q^*} A(u,t) - B(u,t) du,
\end{equation}
the per-share area between orderbook S and D averaged over grid times $t=10, 15, ... , 110$ in each period. \pause
For Flow it is
\begin{equation} \label{eq:ppiflow}
    PPI_{Flow} = \frac{1}{Q^*}\int_t^{t+\Delta t} a(q+r,s) - b(q+r,s) ds,
\end{equation}
where $a$ and $b$ are flow S and D (at time $s$), while $r$ is instantaneous flow rate (at intersection of $a$ and $b$) and 
...
\begin{frame}{H3: Trade Volume Regression}
...
\begin{frame}{Result 3: Trade Volume is lower in Flow format than in CDA.}
\begin{itemize}
% \item Flow market format has 8-9 fewer orders submitted per period than the 32 in CDA ($p < 0.01$).
% \item But the orders are larger, especially in later periods (by $\sim 17$, compared to $\sim$40 in CDA, $p < 0.01$).  
% \item Taken together, these results suggest that the Flow market format shreds orders more effectively than the CDA. 
\item Overall lower trading volume in Flow, by $\sim$ 130 shares in later periods ($p < 0.05$).
\item Volume in Flow trends significantly upward over time (shown elsewhere).
...
\begin{frame}{H4: Efficiency Regressions}
...
\begin{frame}{Result 4: Flow and CDA formats have comparable informational and allocative efficiency}
\begin{itemize}
...
\item Price \textbf{volatility} is much lower in Flow. (Prelim PPI too) \pause
